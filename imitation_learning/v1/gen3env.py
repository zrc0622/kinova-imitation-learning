...
class gen3env(gym.Env):
    ...
    def compute_dis(self,obs):
        end_pose=obs[:3] # 前四位为机械臂位置
        peg_pose=obs[4:7] # 中间的为插头位置
        # print('peg',peg_pose,type(peg_pose))
        hole_pose=obs[7:]  # 后三位为洞的位置
        ...
        return end_peg_dis,peg_hole_dis,grab_dis # 获取几个距离
    ...
    def get_action(self):
        ...
        end_z=end_effect_pose.position.z
        gripper_opening=self.robot.get_gripper_position()
        action=np.array([end_x,end_y,end_z,gripper_opening])
        return action
